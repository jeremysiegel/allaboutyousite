export default class PeriodStrings {
  constructor() {

    var questions = [
      [
        "Q: How will I know when my first period is coming?\n\nA: In general, you may not know that your first period is coming when you get it. When it does happen, you can talk to a trusted adult like a parent, teacher, or school nurse to get a hygiene product like a pad. Friends are usually supportive and willing to share."      
      ],
      [
        "\nQ: Is there a lot of blood?\n\nA: Every person is different, and some people have heavier period flows than others. It may be helpful to know that the blood doesn't come out all at once, usually just a teaspoon (3-5 milliliters) at a time."
      ],
      [
      "\nQ: How often will I get my period?\n\nA: It can vary. Every 28 days is just the average (in fact, only around 15% of women actually have cycles that are exactly that long). Normal periods are normal for your body. Especially at the beginning they will vary, and can be anywhere from 20-45 days apart. Later, they will be come more regular, and you can keep track in your calendar."
      ],
      [
        "\nQ: How long is each period?\n\nA: Again, everyone is different, but they can be anywhere from 2-7 days."
      ],
      [
        "\nQ: What is a tampon? A pad?\n\nA: There are a lot of period products to stop the blood from getting on your clothes. Pads go in the underwear and catch the blood, and tampons go in the vagina and soak up the blood."
      ],
      [
        "\nQ: Does the egg leave the body with the period?\n\nA: No. If there is no sperm present, the egg disintegrates after 24 hours. The egg leaves the ovary halfway through the cycle, at the opposite time that the period happens."
      ]
    ];

    return questions;    
  }
}